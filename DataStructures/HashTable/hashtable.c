...
static void ch_hash_grow(ch_hash *hash) {
  ...
  if (NULL == new_buckets) {
    fprintf(stderr,
            "Cannot resize buckets array. Hash table won't be resized.\n");
    return;
  }
...
void ch_string_free(void *data, void *arg) { free(data); }

void ch_string_print(const void *data) { printf("%s", (const char *)data); }

ch_key_ops ch_key_ops_string = {ch_string_hash, ch_string_cp, ch_string_free,
                                ch_string_eq, NULL};
ch_val_ops ch_val_ops_string = {ch_string_cp, ch_string_free, ch_string_eq,
                                NULL};
